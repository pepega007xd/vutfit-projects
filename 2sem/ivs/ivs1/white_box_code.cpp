...
hash_map_state_code_t hash_map_reserve(hash_map_t *self, size_t size) {
    ...
    hash_map_item_t **new_index =
        (hash_map_item_t **)malloc(size * sizeof(hash_map_t *));
    if (new_index == NULL) {
    ...
    if (self->index != NULL) {
        ...
        size_t idx;
        for (hash_map_item_t *item = self->first; item != NULL;
             item = item->next) {
...
size_t hash_map_size(hash_map_t *self) { return self->used; }

size_t hash_map_capacity(hash_map_t *self) { return self->allocated; }
...
hash_map_state_code_t hash_map_put(hash_map_t *self, const char *key,
    ...
    if (((float)self->used / (float)self->allocated) >=
        HASH_MAP_REALLOCATION_THRESHOLD) {
