...
struct Node {
  ...
  size_t color; ///< celé číslo reprezentující barvu uzlu, výchozí barva je 0 a
                ///< značí neobarveno
 ...
 * Třída umožňuje porovnání dvou hran tak, že i opačně orientované hrany mezi
 * stejnými dvěma uzly jsou si rovny.
 */
class Edge {
public:
  size_t a; ///< id uzlu a
...
public:
  ...
  /**
   * Přidá uzel s daným id do grafu a vrátí ukazatel na vytvořený uzel. Pokud
   * uzel existuje vrátí nullptr. Volající se nestárá o mazání uzlu.
   *
  ...
  /**
   * Provede obarvení uzlů v grafu. Obarvení je uloženo v atributu color v daném
   * uzlu. Nesmí se použít více než graphDegree + 1 barev. Jaký algoritmus pro
   * barvení zvolíte je na Vás. Může být použita heuristika nepřiřazující
   * minimální počet barev, ale musí být splněny testy.
   *
   * Barvením se rozumí, že přiřadíte každému uzlu barvu tak, že sousední uzly
...
protected:
  std::vector<Node> nodes_data;
  std::map<size_t, std::set<size_t>> edges_data;

  /**
   * Vraci optional hodnotu indexu do nodes_data na zaklade node_id.
   * Nalezeno == index, true
   * Nenalezelo == 0, false
   */
  std::pair<size_t, bool> find_node(size_t node_id);

  void color_recursive(Node &node, std::set<size_t> &colors);
};
