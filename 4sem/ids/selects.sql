...
-- select all items with their categories (join 2 tables)
select I.item_name, C.category_name
from Item I, Item_category C
where I.category_id = C.category_id;

-- select all users with the content of their baskets (join 2 tables)
select U.first_name, U.last_name, I.item_name, B.amount
from user_account U, Basket B, Item I
where U.personal_email = B.personal_email and I.item_id = B.item_id;

-- select all users with their orders and the content of each order (join 3 talbes)
select U.first_name, U.last_name, O.order_id, I.item_name, C.amount
from user_account U, shop_order O, order_contains_item C, Item I
where U.personal_email = O.user_account and O.order_id = C.order_id and C.item_id = I.item_id;

-- count, how many items are in each item category, sort by most populated category (GROUP BY)
select C.category_name, count(*) number_of_items
from Item I, Item_category C
where I.category_id = C.category_id
...
select U.first_name, U.last_name, AVG(B.amount) as average_items_in_basket
from user_account U, Basket B, Item I
where U.personal_email = B.personal_email
group by U.first_name, U.last_name;

-- select all users which have at lease one order
select U.*
from user_account U
where exists (select * from shop_order O where O.user_account = U.personal_email);

-- select all items that are in the any user's basket and any of their orders at the same time
select I.item_name, U.first_name, U.last_name
from Item I, User_account U
where I.item_id IN ( -- items in basket of user U
    select I2.item_id
    from Basket B, Item I2
    where U.personal_email = B.personal_email and B.item_id = I2.item_id
)
and I.item_id IN ( -- items in orders of user U
    select I2.item_id
    from shop_order O, order_contains_item C, Item I2
    where U.personal_email = O.user_account and O.order_id = C.order_id and C.item_id = I2.item_id
);

---------- extra queries --------------

-- select all items with below average price
select I.item_name, I.price
from Item I
where I.price < (select AVG(I.price) from Item I);

-- find the item in each user's basket with the maximum amount in that user's basket
select U.first_name, U.last_name, I.item_name, B.amount
from user_account U, Basket B, Item I
where U.personal_email = B.personal_email and I.item_id = B.item_id
and B.amount = (
    select MAX(B.amount)
    from user_account U2, Basket B
    where U.personal_email=U2.personal_email and U2.personal_email = B.personal_email
);
...
grant select,insert,update,delete,references,alter,index,flashback on item to xbrabl04;
grant select,insert,update,delete on item_category to xbrabl04;
grant select,insert,update,delete on basket to xbrabl04;

drop materialized view item_price_below_20k;
drop materialized view total_tems_above_40k;


create MATERIALIZED view item_price_below_20k
...
create materialized view total_tems_above_40k
refresh complete on demand
as 
select 'Products priced above 40k' as description,count(*) as total_count
from item
where price > 40000;
