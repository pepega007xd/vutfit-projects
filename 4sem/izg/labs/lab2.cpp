...
void rasterizeLine(int x1, int y1, int x2, int y2, S_RGBA color) {
    ...
    // #1 : Doplnte kod pro kontrolu vstupu a upravu koordinatu pro ruzne
    // kvadranty.

    // don't draw line with zero length, just draw the point
    if (dx == 0 && dy == 0) {
        setPixelColor(x1, y1, color);
        return;
    }

    // true iff we have to draw the line transformed y <-> x
    bool is_steep = abs(dy) > abs(dx);

    // mirror the line, so that we don't draw the points sparsely
    if (is_steep) {
        swap(x1, y1);
        swap(x2, y2);
    }

    // recompute dx, dy after transformation
    dx = x2 - x1;
    dy = y2 - y1;
    ...
    for (int x = x1; x <= x2; ++x) {
        // #2 : Doplnte kod pro upravu koordinatu pro ruzne kvadranty, pripadne
        // upravte i setPixelColor(...).
        if (is_steep) {
