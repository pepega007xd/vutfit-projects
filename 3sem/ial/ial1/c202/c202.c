...
bool Stack_IsFull(const Stack *stack) {
    // `STACK_SIZE - 1` means `STACK_SIZE` elements in array => full
...
void Stack_Push(Stack *stack, char data) {
    ...
    // topIndex is index of *existing* top element => incrementing first
    stack->topIndex++;
...
void Stack_Dispose(Stack *stack) {
    ...
    // if someone uses deallocated stack, the program will crash instead of UB
    stack->array = NULL;
    stack->topIndex = -1;
}
